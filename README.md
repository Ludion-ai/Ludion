...
These are exported from the main `ludion-router` entry (`setDropinConfig` and
`setConfigSource` are also re-exported from `ludion-router/openai`):

- `setDropinConfig(config)` installs an in-memory config (validated, versioned).
  Nothing is persisted.
- `setConfigSource(source)` swaps in your own `ConfigSource`
...
server-side relay so the key never reaches the browser (see the relay recipes in
[`docs/recipes/`](https://github.com/Ludion-ai/Ludion/tree/main/docs/recipes)).
