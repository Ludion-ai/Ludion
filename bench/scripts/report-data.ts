...
// readme-evidence (README §5)
{
  ...
  // Absolute URLs: README.md is synced byte-for-byte to router/README.md and
  // published to npm, where the package's repository.directory ("router")
  // rewrites relative links to a non-existent router/bench/ path. Absolute
  // links are the only form correct on both the repo root and the npm page.
  blocks.set(
    "readme-evidence",
    `Policy v0 is derived from **${total} archived benchmark runs** across **${devices.length} device/environment configurations** (${devices.join(", ")}), collected with [\`bench/\`](https://github.com/Ludion-ai/Ludion/tree/main/bench) and archived byte-for-byte in [\`bench/results/\`](https://github.com/Ludion-ai/Ludion/tree/main/bench/results). The aggregate is regenerated by script — no number in this README or the report is hand-typed.`,
  );
}
